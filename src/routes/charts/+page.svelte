...
let containerConfig = $state<Chart.ChartConfig>({} satisfies Chart.ChartConfig);
...
<main class="container my-6">
  ...
  <Card.Root>
    ...
    <Card.Content class="pl-12">
      <Chart.Container config={data.availableSpaceReport.containerConfig}>
        <LineChart
          data={data.availableSpaceReport.data}
          x="date"
          xScale={scaleUtc()}
          series={data.availableSpaceReport.chartConfig}
          props={{
            spline: { curve: curveNatural, motion: "tween", strokeWidth: 2 },
            xAxis: {
              format: (v: Date) =>
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description
        >Showing a breakdown of individual disk usages for the current captured
        PC</Card.Description
      >
    </Card.Header>
    <Card.Content class="pl-12">
      <Chart.Container config={data.usedSpaceReport.containerConfig}>
        <LineChart
          data={data.usedSpaceReport.data}
          x="date"
          xScale={scaleUtc()}
          series={data.usedSpaceReport.chartConfig}
          props={{
            spline: { curve: curveNatural, motion: "tween", strokeWidth: 2 },
            xAxis: {
              format: (v: Date) =>
  ...
  <Card.Root>
    ...
    <Card.Content class="pl-12">
      <Chart.Container config={data.usedSpacePctReport.containerConfig}>
        <LineChart
          data={data.usedSpacePctReport.data}
          x="date"
          xScale={scaleUtc()}
          series={data.usedSpacePctReport.chartConfig}
          props={{
            spline: { curve: curveNatural, motion: "tween", strokeWidth: 2 },
            xAxis: {
              format: (v: Date) =>
